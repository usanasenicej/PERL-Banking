...
async function loadLoans() {
    try {
        ...
        loans.forEach(loan => {
            list.innerHTML += `
                <div class="card" style="border-color: ${loan.status === 'pending' ? 'orange' : 'var(--primary)'}">
                    <div class="card-details">
...
function populateAccountDropdowns() {
    const options = accountsData.map(a => `<option value="${a.id}">${a.account_type} (${a.id}) - $${a.balance}</option>`).join('');
...
async function submitTransfer() {
    ...
    try {
        await apiCall('/transactions/transfer', 'POST', { from_account_id: parseInt(fromId), to_account_id: parseInt(toId), amount: parseFloat(amount) });
